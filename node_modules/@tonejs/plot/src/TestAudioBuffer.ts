...
export class TestAudioBuffer {
	...
	static async fromUrl(url: string, channels = 1, sampleRate = 11025): Promise<TestAudioBuffer> {
	...
	static fromTone(buffer: import("tone").ToneAudioBuffer) {
	...
	async resample(sampleRate: number): Promise<TestAudioBuffer> {
		...
		const offlineCtx = new OfflineAudioContext(this._buffer.numberOfChannels,
			this._buffer.duration * sampleRate, sampleRate);
		const resampledBuffer = offlineCtx.createBuffer(this._buffer.numberOfChannels,
			this._buffer.length, this._buffer.sampleRate);
